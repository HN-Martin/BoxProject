...
InstancesModel::InstancesModel ( QObject* parent=NULL )
...
int      InstancesModel::rowCount       ( const QModelIndex& parent ){
  return (cell_) ? cell_->getInstances().size() : 0;
}

int      InstancesModel::ColumnCount    ( const QModelIndex& parent ){
  return 2;
}

QVariant InstancesModel::data           ( const QModelIndex& index, int role ){
